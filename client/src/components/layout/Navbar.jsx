...
const NavItem = () => {
  ...
  return (
    <>
      <div className="items-center gap-3 lg:gap-6 hidden lg:flex">
        {navItem.map((item, index) => (
          <Link key={index} to={item.path}>
            <span
              className={`${
                location.pathname === item.path
                  ? " text-primaryColor underline font-semibold transition-all duration-300"
                  : ""
              } text-sm lg:text-md font-medium uppercase text-gray-600`}
            >
              {item.title}
            </span>
          </Link>
      ...
      <div
        ...
        onClick={() => seOpenSidebar((prev) => !prev)}
      >
        <IoMenu size={30} />
      </div>

      <div
        className={`lg:hidden absolute top-0 left-0 w-full h-[100dvh] bg-white transition-all duration-300 px-2 py-14 z-20 ${
          openSidebar ? "" : " -translate-x-[100%]"
        }`}
      >
        {/* Close Icon  */}
        <div
          className=" absolute left-4 top-7 cursor-pointer"
          onClick={() => seOpenSidebar(false)}
        >
          <IoClose size={30} />
        </div>

        {/* Logo  */}
        <div className="h-28 w-36 mx-auto mb-10">
...
const Navbar = () => {
  ...
  const handleOpenSignInModal = () => setOpenSignInModal((cur) => !cur);

  const [openSignUpModal, setOpenSignUpModal] = React.useState(false);
  const handleOpenSignUpModal = () => setOpenSignUpModal((cur) => !cur);
  ...
  return (
    <>
      <div
        className="px-4 md:px-10 py-1 flex items-center justify-center  xsm:justify-between absolute top-0 w-full left-0 z-30 flex-wrap shadow-lg bg-white"
        data-aos="fade-down"
        data-aos-delay="200"
      >
        <div className="flex items-center gap-2 md:gap-10 lg:gap-20 justify-between w-full xsm:w-auto">
        ...
        <div className=" flex items-center gap-2 sm:gap-4 justify-end w-full xsm:w-auto">
          {isAuthenticated ? (
      ...
      <SignIn
        ...
        signUpHandleOpen={handleOpenSignUpModal}
      />
      <SignUp
        open={openSignUpModal}
        handleOpen={handleOpenSignUpModal}
        signInHandleOpen={handleOpenSignInModal}
      />
    </>
  );
};
